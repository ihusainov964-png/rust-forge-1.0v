...
pub fn draw_advanced_tab(app: &mut RustForgeApp, ui: &mut Ui) {
    ...
    egui::ScrollArea::vertical().auto_shrink([false; 2]).show(ui, |ui| {
        ...
        ui.horizontal(|ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                if secondary_button(ui, "Включить все").clicked() {
                    ...
                    app.push_notification("Все 40 эффектов включены", crate::config::NotificationKind::Info);
                }
                if action_button(ui, "🔥 Максимальный FPS (выключить все)").clicked() {
                    app.config.advanced = AdvancedTweaks::all_off();
                    app.push_notification("Все 40 эффектов отключены", crate::config::NotificationKind::Success);
                }
            });
        });
        ui.add_space(10.0);
        ...
        egui::Grid::new("adv_world").num_columns(2).spacing([24.0, 6.0]).show(ui, |ui| {
            checkbox_tip(ui, &mut app.config.advanced.fog_detail, "Детализация тумана",
                "graphics.fog — плотный туман считается каждый кадр. OFF на дальних дистанциях = чище FPS.");
            checkbox_tip(ui, &mut app.config.advanced.cloud_detail, "Детализация облаков",
                "graphics.clouds — объёмные облака в небе, почти всегда вне поля зрения в PVP.");
            ui.end_row();
            checkbox_tip(ui, &mut app.config.advanced.wind_simulation, "Симуляция ветра",
                "wind.enabled — влияет на деревья/траву/флаги, считается для всей загруженной зоны.");
            checkbox_tip(ui, &mut app.config.advanced.dust_particles, "Пылевые частицы",
                "effects.dust — частицы пыли в воздухе на открытых картах и в карьерах.");
            ui.end_row();
            checkbox_tip(ui, &mut app.config.advanced.ambient_occlusion, "Ambient Occlusion",
                "graphics.ao — мягкие затенения в углах и щелях. Дорогой пост-эффект.");
            checkbox_tip(ui, &mut app.config.advanced.sun_shafts, "Лучи солнца (God Rays)",
        ...
        egui::Grid::new("adv_screen").num_columns(2).spacing([24.0, 6.0]).show(ui, |ui| {
            checkbox_tip(ui, &mut app.config.advanced.vignette, "Виньетирование",
                "graphics.vignette — затемнение краёв экрана, чисто косметика.");
            checkbox_tip(ui, &mut app.config.advanced.chromatic_aberration, "Хроматическая аберрация",
                "graphics.chromaticaberration — цветные разводы по краям. Мешает целиться.");
            ui.end_row();
            checkbox_tip(ui, &mut app.config.advanced.film_grain, "Зерно плёнки (Film Grain)",
                "graphics.filmgrain — шум поверх картинки, постоянный пост-проход.");
            checkbox_tip(ui, &mut app.config.advanced.lens_flare, "Блики объектива (Lens Flare)",
                "graphics.lensflare — блики от солнца/фонарей, лишний рендер-пасс.");
            ui.end_row();
            checkbox_tip(ui, &mut app.config.advanced.screen_space_reflections, "Screen Space Reflections",
                "graphics.ssr — отражения на мокрых поверхностях. Один из самых тяжёлых эффектов.");
            checkbox_tip(ui, &mut app.config.advanced.color_grading_lut, "Цветокоррекция (LUT)",
        ...
        egui::Grid::new("adv_shadow").num_columns(2).spacing([24.0, 6.0]).show(ui, |ui| {
            checkbox_tip(ui, &mut app.config.advanced.contact_shadows, "Контактные тени",
                "graphics.contactshadows — мягкие тени под мелкими объектами. Свободный FPS при отключении.");
            checkbox_tip(ui, &mut app.config.advanced.mesh_shadow_distance, "Тени от мешей на дистанции",
                "mesh.shadowdistance — тени объектов на дальних дистанциях. Агрессивно, но даёт много FPS.");
            ui.end_row();
            checkbox_tip(ui, &mut app.config.advanced.shader_lod_full, "Полная детализация шейдеров",
                "graphics.shaderlod — снижение LOD шейдеров на удалении экономит GPU.");
            checkbox_tip(ui, &mut app.config.advanced.terrain_detail_recalc, "Пересчёт детализации террейна",
                "terrain.idleinterval — как часто пересчитывается детализация ландшафта, пока стоишь.");
            ui.end_row();
            checkbox_tip(ui, &mut app.config.advanced.grass_displacement, "Прогибание травы",
        ...
        egui::Grid::new("adv_particles").num_columns(2).spacing([24.0, 6.0]).show(ui, |ui| {
            checkbox_tip(ui, &mut app.config.advanced.blood_decals, "Декали крови",
                "effects.blooddecals — пятна крови остаются на текстурах и накапливаются в бою.");
            checkbox_tip(ui, &mut app.config.advanced.bullet_impact_decals, "Декали от пуль",
                "effects.bulletdecals — следы от попаданий на стенах и земле.");
            ui.end_row();
            checkbox_tip(ui, &mut app.config.advanced.explosion_particles, "Частицы взрывов",
                "effects.explosionparticles — детализация облака взрыва при рейдах.");
            checkbox_tip(ui, &mut app.config.advanced.muzzle_flash_particles, "Частицы дульной вспышки",
                "effects.muzzleflash — детализированная вспышка выстрела вместо упрощённой.");
            ui.end_row();
            checkbox_tip(ui, &mut app.config.advanced.footstep_dust, "Пыль от шагов",
                "effects.footstepdust — частицы пыли под ногами при беге по земле.");
            checkbox_tip(ui, &mut app.config.advanced.fire_smoke_particles, "Дым от костров/пожаров",
        ...
        egui::Grid::new("adv_physics").num_columns(2).spacing([24.0, 6.0]).show(ui, |ui| {
            checkbox_tip(ui, &mut app.config.advanced.ragdoll_physics_detail, "Детальная физика рэгдоллов",
                "ragdoll.detail — количество физических костей у трупов после смерти.");
            checkbox_tip(ui, &mut app.config.advanced.corpse_extended_lifetime, "Долгое время жизни трупов",
                "corpse.extendedlifetime — трупы дольше остаются на карте и считаются в физике.");
            ui.end_row();
            checkbox_tip(ui, &mut app.config.advanced.cloth_physics, "Физика одежды",
                "graphics.clothphysics — покачивание плащей/тряпок на персонажах.");
            checkbox_tip(ui, &mut app.config.advanced.foliage_wind_sway, "Покачивание листвы на ветру",
                "graphics.foliagesway — анимация качания деревьев и кустов.");
            ui.end_row();
            checkbox_tip(ui, &mut app.config.advanced.deployable_shadow_detail, "Тени построек/декора",
        ...
        egui::Grid::new("adv_audio").num_columns(2).spacing([24.0, 6.0]).show(ui, |ui| {
            checkbox_tip(ui, &mut app.config.advanced.ambience_layer, "Амбиентный звуковой слой",
                "ambience.enabled — фоновая звуковая подложка окружения (ветер, гул, шумы).");
            checkbox_tip(ui, &mut app.config.advanced.wind_audio, "Звук ветра",
                "audio.wind — динамический звук ветра, пересчитывается по погоде.");
            ui.end_row();
            checkbox_tip(ui, &mut app.config.advanced.wildlife_audio, "Звуки живности",
                "audio.wildlife — звуки животных и птиц по всей загруженной зоне.");
            checkbox_tip(ui, &mut app.config.advanced.spatial_voice_processing, "Пространственная обработка голоса",
                "audio.spatialvoice — 3D-позиционирование голосового чата других игроков.");
            ui.end_row();
            checkbox_tip(ui, &mut app.config.advanced.footstep_reverb, "Реверберация шагов",
        ...
        egui::Grid::new("adv_render").num_columns(2).spacing([24.0, 6.0]).show(ui, |ui| {
            checkbox_tip(ui, &mut app.config.advanced.water_simulation_detail, "Детализация симуляции воды",
                "water.simdetail — волны и симуляция поверхности воды в реальном времени.");
            checkbox_tip(ui, &mut app.config.advanced.super_sampling, "Супер-сэмплинг (SSAA)",
                "gfx.ssaa — самый дорогой вид сглаживания, рендерит кадр в повышенном разрешении.");
            ui.end_row();
            checkbox_tip(ui, &mut app.config.advanced.post_process_aa, "Пост-процессное сглаживание",
                "effects.aa_quality — дополнительный AA-проход поверх основного сглаживания.");
            checkbox_tip(ui, &mut app.config.advanced.prop_pooling_optimization, "Пул объектов окружения",
                "graphics.proppooling — держит в памяти доп. объекты для быстрого спавна декора.");
            ui.end_row();
            checkbox_tip(ui, &mut app.config.advanced.camera_shake, "Тряска камеры",
                "graphics.camerashake — тряска экрана при взрывах и попаданиях. Мешает прицеливаться.");
            checkbox_tip(ui, &mut app.config.advanced.combat_text_popups, "Всплывающий текст урона",
                "ui.combattext — цифры урона над целью, лишняя перерисовка UI в бою.");
            ui.end_row();
            checkbox_tip(ui, &mut app.config.advanced.loot_glow_effects, "Свечение лута",
